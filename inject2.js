...
// 1. Update openGameDetail signature
let target1 = 'function openGameDetail(kategori) {';
let replacement1 = `function openGameDetail(kategori, overrideGame = null, returnView = 'grid', preselectProductId = null) {`;
code = code.replace(target1, replacement1);

// 2. Update overrideGame logic inside openGameDetail
let target2 = 'const game = TOPUP_GAMES.find(g => g.kategori === kategori);';
let replacement2 = `const game = overrideGame || TOPUP_GAMES.find(g => g.kategori === kategori);`;
code = code.replace(target2, replacement2);

// 3. Update twState
let target3 = `        promo: null
    };`;
let target3Lf = `        promo: null\n    };`;
let target3CrLf = `        promo: null\r\n    };`;
let replacement3 = `        promo: null,
        returnView: returnView
    };`;
if (code.includes(target3)) code = code.replace(target3, replacement3);
else if (code.includes(target3Lf)) code = code.replace(target3Lf, replacement3);
else if (code.includes(target3CrLf)) code = code.replace(target3CrLf, replacement3);

// 4. Update the end of openGameDetail
let target4 = `    renderTopupProductGrid();
...
let target4Lf = target4.replace(/\r\n/g, '\n');
let replacement4 = `    renderTopupProductGrid();
    renderTopupPaymentGrid();
...
else if (code.includes(target4Lf)) code = code.replace(target4Lf, replacement4);

// 5. Update closeGameDetail
let target5 = `function closeGameDetail() {
...
let target5Lf = target5.replace(/\r\n/g, '\n');
let replacement5 = `function closeGameDetail() {
    document.getElementById("topupDetail").classList.add("hidden");
...
else if (code.includes(target5Lf)) code = code.replace(target5Lf, replacement5);

// 6. Update loadOneStopCatalog in bootstrapApp
let target6 = `        loadLeaderboard(),
        checkPaymentReturn(),
        initMusicPlayer()`;
let target6Lf = target6.replace(/\r\n/g, '\n');
let replacement6 = `        loadLeaderboard(),
        checkPaymentReturn(),
        initMusicPlayer(),
        loadOneStopCatalog()`;
if (code.includes(target6)) code = code.replace(target6, replacement6);
else if (code.includes(target6Lf)) code = code.replace(target6Lf, replacement6);

// 7. Append One Stop Solution code at the end
const oneStopCode = `
// ==========================================
// ONE STOP SOLUTION
// ==========================================
