...
async function resolveChromeExecutable() {
  ...
  if (configured) {
    if (!fs.existsSync(configured)) {
      throw new Error(`CHROME_EXECUTABLE_PATH tidak ditemukan: ${configured}`);
    }
    return configured;
  }
  ...
  if (!executable) {
    throw new Error("Chrome/Chromium tidak ditemukan. Atur CHROME_EXECUTABLE_PATH di environment backend.");
  }
  return executable;
}
